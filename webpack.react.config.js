...
try{ manifest_json = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'dist/manifest.json'),{encoding:'utf-8'}));  } catch (e) {}
let tsconf = {};
try{
...
//        modulesDirectories: ['node_modules'],
    ...
    output:
    {
        path: path.resolve(__dirname, 'distCompiled'),
        filename: 'bundle.js'
    },
    module: {
        rules: [{
                test:  (modulePath0) => {
                    ...
                    for(let excludedModule of excludedModules)
                        if(modulePath.includes(excludedModule))
                            return true;
                    return false;
                },
                use: 'null-loader',
